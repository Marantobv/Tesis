...
# Split post-2021 data into train and test sets
train_size = int(0.8 * len(X_post_2021))
X_train_post_2021, X_test_post_2021 = X_post_2021[:train_size], X_post_2021[train_size:]
y_train_post_2021, y_test_post_2021 = y_post_2021[:train_size], y_post_2021[train_size:]

# Create DataLoaders
train_loader_pre_2021 = DataLoader(TensorDataset(X_pre_2021, y_pre_2021), batch_size=32, shuffle=True)
train_loader_post_2021 = DataLoader(TensorDataset(X_train_post_2021, y_train_post_2021), batch_size=32, shuffle=True)
test_loader_post_2021 = DataLoader(TensorDataset(X_test_post_2021, y_test_post_2021), batch_size=32, shuffle=False)
...
train_model(model, train_loader_post_2021, EPOCHS_POST_2021, criterion, optimizer, device)
...
with torch.no_grad():
    for sequences, labels in test_loader_post_2021:
        sequences, labels = sequences.to(device), labels.to(device)
        outputs = model(sequences)
...
y_pred = scaler_close.inverse_transform(y_pred.reshape(-1, 1)).flatten()
y_true = scaler_close.inverse_transform(y_true.reshape(-1, 1)).flatten()
