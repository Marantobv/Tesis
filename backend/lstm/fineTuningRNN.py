...
model = RNNModel(input_size=input_size_train, hidden_size=hidden_size, num_layers=num_layers, output_size=output_size)
...
class RNNModelFineTune(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size):
        super(RNNModelFineTune, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.rnn = nn.RNN(input_size, hidden_size, num_layers, batch_first=True, nonlinearity='relu')
        self.fc = nn.Linear(hidden_size, output_size)
...
model = RNNModelFineTune(input_size=input_size_finetune, hidden_size=hidden_size, num_layers=num_layers, output_size=output_size)
model.to(device)
...
#torch.save(model.state_dict(), 'fineTuning_model_RNN.pth')
...
plt.title('Precio Real vs Precio Predicho de las Acciones')
plt.xlabel('Tiempo')
plt.ylabel('Precio')
plt.legend()
plt.grid(True)

plt.savefig('real_vs_predicted_prices_RNN.png')
plt.close()

print("Gráfico guardado como 'real_vs_predicted_prices_RNN.png'")
